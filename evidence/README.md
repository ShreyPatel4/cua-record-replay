...
- `step_NN.png`: the screen after step NN, or where the run stopped if step NN never finished.
  When the step had already finished, the stopping screen is `step_NN_failure.png`. A run that stops on a screen, failure or business outcome, adds `a11y_NN.json`, the
  accessibility snapshot there, with dollar amounts masked once a
...
- `step_NN.png`: the screen the model saw at turn NN. Turns that call `request_confirmation` take
  no observation, so their numbers are missing from the sequence.
